...
- `pylock.toml` is derived from `uv.lock` to support a wider range of ecosystem tools. Update with `uv export --all-extras --locked --output-file=pylock.toml`.

After updating dependencies in `pyproject.toml`, make sure to regenerate `uv.lock` and `pylock.toml` to keep them up to date. Both pre-commit hooks and the CI pipeline contain checks for these.
